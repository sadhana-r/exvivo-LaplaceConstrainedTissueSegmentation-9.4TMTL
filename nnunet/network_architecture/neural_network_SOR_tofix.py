...
class SegmentationNetwork(NeuralNetwork):
            ...
            with torch.no_grad():
                if self.conv_op == nn.Conv3d:
                    if use_sliding_window:
                        print("calling internal_predict_3D_3Dconv_tiled")
    ...
    def _internal_predict_3D_3Dconv_tiled(self, x: np.ndarray, step_size: float, do_mirroring: bool, mirror_axes: tuple,
        ...
        for x in steps[0]:
            ...
            for y in steps[1]:
                ...
                for z in steps[2]:
                    ...
                    predicted_patch, predicted_patch_laplace  = self._internal_maybe_mirror_and_pred_3D(data[None, :, lb_x:ub_x, lb_y:ub_y, lb_z:ub_z], 
        ...
        if regions_class_order is None:
            ...
            predicted_laplace = class_probabilities.argmax(0)
        else:
            if all_in_gpu:
                class_probabilities_here = class_probabilities.detach().cpu().numpy()
                laplace_probabilities_here = laplace_probabilities.detach().cpu().numpy()
            else:
                class_probabilities_here = class_probabilities
                laplace_probabilities_here = laplace_probabilities

            predicted_segmentation = np.zeros(class_probabilities_here.shape[1:], dtype=np.float32)
            predicted_laplace = np.zeros(laplace_probabilities_here.shape[1:], dtype=np.float32)
            for i, c in enumerate(regions_class_order):
                predicted_segmentation[class_probabilities_here[i] > 0.5] = c
                predicted_laplace[laplace_probabilities_here[i] > 0.5] = c
        ...
        if all_in_gpu:
            ...
            laplace_probabilities = laplace_probabilities.detach().cpu().numpy()

        if verbose: print("prediction done")
        return [predicted_segmentation, predicted_laplace], [class_probabilities, laplace_probabilities]
    ...
    def _internal_maybe_mirror_and_pred_3D(self, x: Union[np.ndarray, torch.tensor], mirror_axes: tuple,
        ...
        return result_torch.squeeze(), result_torch_laplace.squeeze()
